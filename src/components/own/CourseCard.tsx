  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl overflow-hidden border border-white/50 dark:border-slate-700/50 hover:shadow-2xl transition-all duration-300 transform max-lg:hover:scale-105 h-[97%] flex flex-col my-3">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        {/* Pointed Text Items */}
        {course.pointedText && course.pointedText.length > 0 && (
          <div className="mb-4">
            <div className="flex flex-wrap gap-2">
              {course.pointedText.slice(0, 3).map((point, index) => (
                <Badge key={index} variant="secondary" className="text-xs">
                  {point}
                </Badge>
              ))}
              {course.pointedText.length > 3 && (
                <Badge variant="secondary" className="text-xs">
                  +{course.pointedText.length - 3} more
                </Badge>
              )}
            </div>
          </div>
        ...
        <div className="flex gap-3 mt-auto">
          <Link href={`/course/${course.id}`} className="flex-1">
            <Button className="w-full bg-red-700 hover:bg-[#9a0000] text-white px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
              See Details
            </Button>
          </Link>
          <Button
            // className="flex-1 bg-white"
            className="flex-1 bg-white border-2 border-red-700 dark:border-gray-800 dark:hover:border-white text-red-700 hover:bg-red-700 hover:text-white px-6 py-3 rounded-xl text-base font-semibold shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
          >
            Enroll Now
          </Button>
        </div>
