// Use NEXT_PUBLIC_ prefix for environment variables accessible in browser
export const API_BASE_URL = process.env.NEXT_PUBLIC_Backend_API_URL || process.env.Backend_API_URL || 'http://localhost:8001';

export const API_URLS = {
  signup: {
    byId: (id: number) => `${API_BASE_URL}/api/auth/signup/${id}`,
    create: `${API_BASE_URL}/api/auth/signup`,
    update: (id: number) => `${API_BASE_URL}/api/auth/signup/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/auth/signup/${id}`,
  },
  auth: {
    login: `${API_BASE_URL}/api/auth/login`,
    logout: `${API_BASE_URL}/api/auth/logout`,
    profile: `${API_BASE_URL}/api/auth/profile`,
    register: `${API_BASE_URL}/api/auth/signup`,
    forgotPassword: `${API_BASE_URL}/api/auth/forgot-password`,
    resetPassword: `${API_BASE_URL}/api/auth/reset-password`,
    verifyOtp: `${API_BASE_URL}/api/auth/verify-otp`,
  },
  users: {
    getAll: `${API_BASE_URL}/api/auth/users`,
    getById: (id: number) => `${API_BASE_URL}/api/auth/users/${id}`,
    create: `${API_BASE_URL}/api/auth/users/create`,
    update: (id: number) => `${API_BASE_URL}/api/auth/users/${id}`,
    updateRole: (id: number) => `${API_BASE_URL}/api/auth/users/${id}/role`,
    updateStatus: (id: number) => `${API_BASE_URL}/api/auth/users/${id}/status`,
    delete: (id: number) => `${API_BASE_URL}/api/auth/users/${id}`,
    bulkAction: `${API_BASE_URL}/api/auth/users/bulk-action`,
  },
  notices: {
    getAll: `${API_BASE_URL}/api/notices`,
    getById: (id: number) => `${API_BASE_URL}/api/notices/${id}`,
    create: `${API_BASE_URL}/api/notices`,
    update: (id: number) => `${API_BASE_URL}/api/notices/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/notices/${id}`,
    getUnreadCount: `${API_BASE_URL}/api/notices/unread/count`,
  },
  admins: {
    getAll: `${API_BASE_URL}/api/auth/admins`,
    promote: `${API_BASE_URL}/api/auth/admins/promote`,
    demote: `${API_BASE_URL}/api/auth/admins/demote`,
  },
  courses: {
    getAll: `${API_BASE_URL}/api/courses`,
    getById: (id: number) => `${API_BASE_URL}/api/courses/${id}`,
    create: `${API_BASE_URL}/api/courses`,
    update: (id: number) => `${API_BASE_URL}/api/courses/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/courses/${id}`,
  },
  books: {
    getAll: `${API_BASE_URL}/api/books`,
    getById: (id: number) => `${API_BASE_URL}/api/books/${id}`,
    create: `${API_BASE_URL}/api/books`,
    update: (id: number) => `${API_BASE_URL}/api/books/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/books/${id}`,
  },
  examCourses: {
    getAll: `${API_BASE_URL}/api/examCourses`,
    getById: (id: number) => `${API_BASE_URL}/api/examCourses/${id}`,
    create: `${API_BASE_URL}/api/examCourses`,
    update: (id: number) => `${API_BASE_URL}/api/examCourses/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/examCourses/${id}`,
  },
  groups: {
    getAll: `${API_BASE_URL}/api/groups`,
    getById: (id: number) => `${API_BASE_URL}/api/groups/${id}`,
    create: `${API_BASE_URL}/api/groups`,
    update: (id: number) => `${API_BASE_URL}/api/groups/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/groups/${id}`,
  },
  subjects: {
    getAll: `${API_BASE_URL}/api/subjects`,
    getById: (id: number) => `${API_BASE_URL}/api/subjects/${id}`,
    create: `${API_BASE_URL}/api/subjects`,
    update: (id: number) => `${API_BASE_URL}/api/subjects/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/subjects/${id}`,
  },
  chapters: {
    getAll: `${API_BASE_URL}/api/chapters`,
    getById: (id: number) => `${API_BASE_URL}/api/chapters/${id}`,
    create: `${API_BASE_URL}/api/chapters`,
    update: (id: number) => `${API_BASE_URL}/api/chapters/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/chapters/${id}`,
  },
  subchapters: {
    getAll: `${API_BASE_URL}/api/subchapters`,
    getById: (id: number) => `${API_BASE_URL}/api/subchapters/${id}`,
    create: `${API_BASE_URL}/api/subchapters`,
    update: (id: number) => `${API_BASE_URL}/api/subchapters/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/subchapters/${id}`,
  },
  questions: {
    getAll: `${API_BASE_URL}/api/questions`,
    getById: (id: number) => `${API_BASE_URL}/api/questions/${id}`,
    create: `${API_BASE_URL}/api/questions`,
    update: (id: number) => `${API_BASE_URL}/api/questions/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/questions/${id}`,
  },
  exams: {
    getAll: `${API_BASE_URL}/api/exams`,
    getById: (id: number) => `${API_BASE_URL}/api/exams/${id}`,
    create: `${API_BASE_URL}/api/exams`,
    update: (id: number) => `${API_BASE_URL}/api/exams/${id}`,
    delete: (id: number) => `${API_BASE_URL}/api/exams/${id}`,
    start: (id: number) => `${API_BASE_URL}/api/exams/${id}/start`,
    submit: (id: number) => `${API_BASE_URL}/api/exams/${id}/submit`,
    adminReport: `${API_BASE_URL}/api/exams/admin/report`,
    statistics: `${API_BASE_URL}/api/exams/statistics`,
    participation: `${API_BASE_URL}/api/exams/participation`,
  },
  systemSettings: {
    getAll: `${API_BASE_URL}/api/system-settings`,
    getPublic: `${API_BASE_URL}/api/system-settings/public`,
    getByCategory: (category: string) => `${API_BASE_URL}/api/system-settings/categories/${category}`,
    getByKey: (key: string) => `${API_BASE_URL}/api/system-settings/${key}`,
    getValue: (key: string) => `${API_BASE_URL}/api/system-settings/${key}/value`,
    create: `${API_BASE_URL}/api/system-settings`,
    update: (key: string) => `${API_BASE_URL}/api/system-settings/${key}`,
    updateValue: (key: string) => `${API_BASE_URL}/api/system-settings/${key}/value`,
    delete: (key: string) => `${API_BASE_URL}/api/system-settings/${key}`,
    initialize: `${API_BASE_URL}/api/system-settings/initialize`,
  },
};